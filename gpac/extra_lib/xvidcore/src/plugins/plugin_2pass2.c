...
rc_2pass2_destroy(rc_2pass2_t * rc, xvid_plg_destroy_t * destroy)
{
	DPRINTF(XVID_DEBUG_RC, "[xvid rc] -- target_total:%lld desired_total:%.2f (%.2f%%) actual_total:%.2f (%.2f%%)\n",
			rc->target,
			rc->desired_total,
			100*rc->desired_total/(double)(int64_t)rc->target,
			rc->real_total,
			100*rc->real_total/(double)(int64_t)rc->target);
...
first_pass_scale_curve_internal(rc_2pass2_t *rc)
{
	...
	scaler = (double)(int64_t)(target - total_invariant) / (double)(int64_t)(rc->tot_weighted);
...
scaled_curve_apply_advanced_parameters(rc_2pass2_t * rc)
{
	 ...
	 * movie_curve) */
	rc->pb_iboost_tax_ratio  = (double)(int64_t)rc->tot_scaled_length[XVID_TYPE_PVOP-1];
	rc->pb_iboost_tax_ratio += (double)(int64_t)rc->tot_scaled_length[XVID_TYPE_BVOP-1];
	...
	for(i=0; i<3; i++) {
		...
		if (rc->count[i] == 0 || rc->pb_iboost_tax_ratio == 0) {
			...
			rc->avg_length[i] = (double)(int64_t)rc->tot_scaled_length[i];
