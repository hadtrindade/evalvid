...
int av_aes_init(AVAES *a, uint8_t *key, int key_bits, int decrypt) {
    ...
    uint8_t  log8[256];
    uint8_t alog8[512];
	int inttab1[] = {0xe, 0x9, 0xd, 0xb};
	int inttab2[] = {0x2, 0x1, 0x1, 0x3};
    ...
    if(!enc_multbl[4][1023]){
        ...
        init_multbl2(dec_multbl[0], inttab1, log8, alog8, inv_sbox);
        init_multbl2(enc_multbl[0], inttab2, log8, alog8, sbox);
    }
