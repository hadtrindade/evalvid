...
#ifdef CONFIG_MSVC
		svq1_pmv_t *pmv = (svq1_pmv_t*) _alloca((width/8+3)*sizeof(svq1_pmv_t));
#else
...
#ifdef CONFIG_MSVC
            uint8_t *src = (uint8_t *)_alloca(stride*16*sizeof(uint8_t));
#else
...
#ifdef CONFIG_MSVC
            uint8_t *temp = (uint8_t *)_alloca(16*stride*sizeof(uint8_t));
#else
...
static enum PixelFormat svq1_pf[2] = {PIX_FMT_YUV410P, -1} ;
