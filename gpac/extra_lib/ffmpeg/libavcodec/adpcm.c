...
#ifdef CONFIG_MSVC
	TrellisPath *paths = (TrellisPath *)_alloca(max_paths*sizeof(TrellisPath));
	TrellisPath *p;
    TrellisNode *node_buf[2], *nodep_buf[2];
    TrellisNode **nodes = nodep_buf[0]; // nodes[] is always sorted by .ssd
    TrellisNode **nodes_next = nodep_buf[1];
    int pathn = 0, froze = -1, i, j, k;
	node_buf[0] = (TrellisNode *)_alloca(frontier*sizeof(TrellisNode));
	node_buf[1] = (TrellisNode *)_alloca(frontier*sizeof(TrellisNode));
	nodep_buf[0] = (TrellisNode *)_alloca(frontier*sizeof(TrellisNode));
	nodep_buf[1] = (TrellisNode *)_alloca(frontier*sizeof(TrellisNode));
#else
...
#ifdef CONFIG_MSVC
                uint8_t *buf[2];
				buf[0] = (uint8_t*)_alloca(n*8*sizeof(uint8_t));
				buf[1] = (uint8_t*)_alloca(n*8*sizeof(uint8_t));
#else
                uint8_t buf[2][n*8];
#endif
...
#ifdef CONFIG_MSVC
            uint8_t *buf[2];
			buf[0] = (uint8_t*)_alloca(n*sizeof(uint8_t));
			buf[1] = (uint8_t*)_alloca(n*sizeof(uint8_t));
#else
            uint8_t buf[2][n];
#endif
            if(avctx->channels == 1) {
...
#ifdef CONFIG_MSVC
            uint8_t *buf[2];
			buf[0] = (uint8_t*)_alloca(n*2*sizeof(uint8_t));
			buf[1] = (uint8_t*)_alloca(n*2*sizeof(uint8_t));
#else
            uint8_t buf[2][n*2];
#endif
            n *= 2;
