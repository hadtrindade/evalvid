...
const char *av_get_string(void *obj, const char *name, const AVOption **o_out, char *buf, int buf_len){
    ...
    switch(o->type){
#ifdef __SYMBIAN32__
	case FF_OPT_TYPE_FLAGS:     sprintf(buf, "0x%08X",*(int    *)dst);break;
    case FF_OPT_TYPE_INT:       sprintf(buf, "%d" , *(int    *)dst);break;
    case FF_OPT_TYPE_INT64:     sprintf(buf, "%"PRId64, *(int64_t*)dst);break;
    case FF_OPT_TYPE_FLOAT:     sprintf(buf, "%f" , *(float  *)dst);break;
    case FF_OPT_TYPE_DOUBLE:    sprintf(buf, "%f" , *(double *)dst);break;
    case FF_OPT_TYPE_RATIONAL:  sprintf(buf, "%d/%d", ((AVRational*)dst)->num, ((AVRational*)dst)->den);break;
#else
