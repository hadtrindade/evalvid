...
static void init_pow2table(COOKContext *q){
    ...
    q->pow2tab[63] = 1.0;
    for (i=1 ; i<64 ; i++){
        q->pow2tab[63+i]=(float)(int64_t)((uint64_t)1<<i);
        q->pow2tab[63-i]=1.0/(float)(int64_t)((uint64_t)1<<i);
    }
}
...
static void init_rootpow2table(COOKContext *q){
    ...
    for (i=1 ; i<64 ; i++){
        q->rootpow2tab[63+i]=sqrt((float)(int64_t)((uint64_t)1<<i));
        q->rootpow2tab[63-i]=sqrt(1.0/(float)(int64_t)((uint64_t)1<<i));
    }
}
