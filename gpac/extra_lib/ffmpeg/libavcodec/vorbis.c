...
static inline __av_deb(const char* fmt,...) { }
#  define AV_DEBUG __av_deb
# else
#  define AV_DEBUG(...)
# endif
#endif
...
#ifdef CONFIG_MSVC
            uint_fast16_t *codebook_multiplicands =  (uint_fast16_t *) _alloca(codebook_lookup_values*sizeof(uint_fast16_t));
#else
...
#ifdef CONFIG_MSVC
    uint_fast16_t *floor1_Y = (uint_fast16_t ) _alloca(vf->x_list_dim*sizeof(uint_fast16_t));
    uint_fast16_t *floor1_Y_final = (uint_fast16_t *)_alloca(vf->x_list_dim*sizeof(uint_fast16_t));
    int *floor1_flag = (int *)_alloca(vf->x_list_dim*sizeof(int));
#else
...
#ifdef CONFIG_MSVC
    uint_fast8_t *classifs = (uint_fast8_t *)_alloca(ptns_to_read*vc->audio_channels*sizeof(uint_fast8_t));
#else
...
static int vorbis_parse_audio_packet(vorbis_context *vc) {
    ...
    int_fast32_t i,j;
#ifdef CONFIG_MSVC
	uint_fast8_t *no_residue = (uint_fast8_t *)_alloca(vc->audio_channels*sizeof(uint_fast8_t));
    uint_fast8_t *do_not_decode = (uint_fast8_t *)_alloca(vc->audio_channels*sizeof(uint_fast8_t));
    uint_fast8_t *res_chan = (uint_fast8_t *)_alloca(vc->audio_channels*sizeof(uint_fast8_t));
#else
