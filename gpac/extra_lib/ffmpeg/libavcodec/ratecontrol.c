    ...
    if(!(s->flags&CODEC_FLAG_PASS2)){
        ...
        if(s->avctx->rc_initial_cplx){
            for(i=0; i<60*30; i++){
                ...
                rcc->i_cplx_sum [rce.pict_type] += rce.i_tex_bits*(int64_t) rce.qscale;
                rcc->p_cplx_sum [rce.pict_type] += rce.p_tex_bits*(int64_t) rce.qscale;
...
//printf("%f %f %f last:%d var:%d type:%d//\n", q, rce->new_qscale, br_compensation, s->frame_bits, var, pict_type);
        ...
        rcc->i_cplx_sum [pict_type] += rce->i_tex_bits*(int64_t) rce->qscale;
        rcc->p_cplx_sum [pict_type] += rce->p_tex_bits*(int64_t) rce->qscale;
...
//printf("%d %f\n", rce->new_bits, blured_qscale[i]);
        ...
        */
        if(expected_bits > (int64_t) all_available_bits) {
            rate_factor-= step;
            ++toobig;
        }
    }
    ...
    av_log(s->avctx, AV_LOG_DEBUG,
        ...
        s->bit_rate,
        (int)(expected_bits / ((double)(int64_t) all_available_bits/s->bit_rate)));
    av_log(s->avctx, AV_LOG_DEBUG,
    ...
    if (toobig == 0) {
        ...
        return -1;
    } else if (fabs(expected_bits/(int64_t) all_available_bits - 1.0) > 0.01) {
        av_log(s->avctx, AV_LOG_ERROR,
