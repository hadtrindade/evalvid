...
#ifdef CONFIG_MSVC
	uint64_t *counts = (uint64_t *)_alloca(2*size*sizeof(uint64_t ));
    int *up=(int *)_alloca(2*size*sizeof(int));
#else
...
static enum PixelFormat hyuv_pf[2]={PIX_FMT_YUV422P, -1};
static enum PixelFormat ffyuv_pf[3]={PIX_FMT_YUV420P, PIX_FMT_YUV422P, -1};
