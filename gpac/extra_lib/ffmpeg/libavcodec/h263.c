...
static void aspect_to_info(MpegEncContext * s, AVRational aspect){
    ...
    if(aspect.num==0) aspect= _AVRational(1,1);
...
void h263_encode_picture_header(MpegEncContext * s, int picture_number)
{
    ...
    if(s->h263_plus){
        for(i=0; i<2; i++){
            int div, error;
            div= (s->avctx->time_base.num*INT64_C(1800000) + INT64_C(500)*s->avctx->time_base.den) / ((INT64_C(1000)+i)*s->avctx->time_base.den);
            div= clip(1, div, 127);
            error= FFABS(s->avctx->time_base.num*INT64_C(1800000) - (INT64_C(1000)+i)*s->avctx->time_base.den*div);
            if(error < best_error){
                best_error= error;
                best_divisor= div;
                best_clock_code= i;
...
int h263_decode_picture_header(MpegEncContext *s)
{
    ...
    */
    ...
    if (format != 7 && format != 6) {
        ...
        s->width = width;
        s->height = height;
        s->avctx->sample_aspect_ratio= _AVRational(12,11);
        s->avctx->time_base= _AVRational(1001, 30000);
    } else {
        int ufep;
