...
static void put_guid(ByteIOContext *s, const ffGUID *g)
{
    assert(sizeof(*g) == 16);
...
static int64_t put_header(ByteIOContext *pb, const ffGUID *g)
{
    int64_t pos;
...
static const AVCodecTag* asf_t[]={codec_asf_bmp_tags, codec_bmp_tags, codec_wav_tags, 0};
