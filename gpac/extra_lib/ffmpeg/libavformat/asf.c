...
static const ffGUID index_guid = {
...
if (!memcmp(g, &cmp, sizeof(ffGUID))) \
    printf("(ffGUID: %s) ", #cmp)

static void print_guid(const ffGUID *g)
{
    int i;
...
static void get_guid(ByteIOContext *s, ffGUID *g)
{
    assert(sizeof(*g) == 16);
...
static int asf_probe(AVProbeData *pd)
{
    ...
    if (!memcmp(pd->buf, &asf_header, sizeof(ffGUID)))
...
static int asf_read_header(AVFormatContext *s, AVFormatParameters *ap)
{
    ...
    get_guid(pb, &g);
    if (memcmp(&g, &asf_header, sizeof(ffGUID)))
        goto fail;
    get_le64(pb);
    get_le32(pb);
    get_byte(pb);
    ...
    for(;;) {
        ...
        if (gsize < 24)
            goto fail;
        if (!memcmp(&g, &file_header, sizeof(ffGUID))) {
        ...
        } else if (!memcmp(&g, &stream_header, sizeof(ffGUID))) {
            ...
            if (!memcmp(&g, &audio_stream, sizeof(ffGUID))) {
                type = CODEC_TYPE_AUDIO;
            } else if (!memcmp(&g, &video_stream, sizeof(ffGUID))) {
                type = CODEC_TYPE_VIDEO;
            } else if (!memcmp(&g, &command_stream, sizeof(ffGUID))) {
                type = CODEC_TYPE_UNKNOWN;
            } else if (!memcmp(&g, &ext_stream_embed_stream_header, sizeof(ffGUID))) {
            ...
            if (test_for_ext_stream_audio) {
                get_guid(pb, &g);
                if (!memcmp(&g, &ext_stream_audio_stream, sizeof(ffGUID))) {
                    type = CODEC_TYPE_AUDIO;
                    get_guid(pb, &g);
                    get_le32(pb);
                    get_le32(pb);
...
//                av_log(NULL, AV_LOG_DEBUG, "size:%d tsize:%d sizeX:%d\n", size, total_size, sizeX);
        ...
        if (st->codec->extradata_size && (st->codec->bits_per_sample <= 8)) {
        ...
        } else if (!memcmp(&g, &data_header, sizeof(ffGUID))) {
            ...
            break;
        } else if (!memcmp(&g, &comment_header, sizeof(ffGUID))) {
            ...
            url_fskip(pb, len5);
       } else if (!memcmp(&g, &extended_content_header, sizeof(ffGUID))) {
                int desc_count, i;
        ...
        } else if (!memcmp(&g, &ext_stream_header, sizeof(ffGUID))) {
        ...
        } else if (!memcmp(&g, &head1_guid, sizeof(ffGUID))) {
            int v1, v2;
            get_guid(pb, &g);
            v1 = get_le32(pb);
            v2 = get_le16(pb);
#if 0
        } else if (!memcmp(&g, &codec_comment_header, sizeof(ffGUID))) {
            int len, v1, n, num;
            char str[256], *q;
            char tag[16];
...
static void asf_build_simple_index(AVFormatContext *s, int stream_index)
{
    ...
    get_guid(&s->pb, &g);
    if (!memcmp(&g, &index_guid, sizeof(ffGUID))) {
        gsize = get_le64(&s->pb);
        get_guid(&s->pb, &g);
        itime=get_le64(&s->pb);
        pct=get_le32(&s->pb);
