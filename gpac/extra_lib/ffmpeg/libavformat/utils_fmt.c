...
#ifdef __SYMBIAN32__
        sprintf(stream_format_name, "%s_stream", fmt->name);
#else
...
static int64_t lsb2full(int64_t lsb, int64_t last_ts, int lsb_bits){
    int64_t mask = lsb_bits < 64 ? (INT64_C(1) <<lsb_bits)-1 : INT64_C(-1);
...
static int get_std_framerate(int i){
	static int std_rates[] = {24,30,60,12,15};
    if(i<60*12) return i*1001;
	else        return std_rates[i-60*12]*1000*12;
}
...
int av_find_stream_info(AVFormatContext *ic)
{
    ...
    for(i=0;i<MAX_STREAMS;i++) {
		duration_error[i] = av_mallocz(sizeof(double)*MAX_STD_TIMEBASES);
	}
...
/*            if(!st->time_base.num)
    ...
    for(;;) {
        ...
        for(i=0;i<ic->nb_streams;i++) {
            ...
            if(   st->codec->time_base.den >= INT64_C(101)*st->codec->time_base.num
...
//                    memset(duration_error, 0, sizeof(duration_error));
    ...
    for(i=0;i<ic->nb_streams;i++) {
        ...
        if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
            ...
            if(duration_count[i]
               && (st->codec->time_base.num*INT64_C(101) <= st->codec->time_base.den || st->codec->codec_id == CODEC_ID_MPEG2VIDEO) /*&&
